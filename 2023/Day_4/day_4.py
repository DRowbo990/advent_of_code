...
card_count = [1] * len(
    lines
)  # for PART 2: list of every card's number of wins > initalize as 1
...
for index, line in enumerate(lines):
    ...
    for x, y in enumerate(winning_numbers):
        for j in range(len(my_numbers)):
            if y == my_numbers[j]:
                matches.append(x)
    cards_worth.append(2 ** (len(matches) - 1)) if len(
        matches
    ) > 0 else cards_worth.append(
        0
    )  # worth = 1 x 2^n-1
